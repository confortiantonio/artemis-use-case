...
/* Caso d'uso relativo a un Push Server usando code statiche definite in broker.xml

broker.xml usato in /sviluppo/amq-broker/amq-broker-7.8.0/instances/amq0/etc:

         <address-setting match="ATADDRESS.1">
            <default-last-value-key>MESSAGE_KEY</default-last-value-key>
            <default-non-destructive>true</default-non-destructive>
         </address-setting>

        <address name="ATADDRESS.1">
            <multicast>
               <queue name="Q1" />
            </multicast>
        </address>
...
public:
    ...
    HelloWorldConsumer(const std::string& brokerURI, int numMessages, bool useTopic = false, bool sessionTransacted = false, int waitMillis = 15 * 60 * 1000) :
        latch(1),
    ...
    virtual void run() {
        ...
        try {
            ...
            if (useTopic) {
                //destination = session->createTopic("TEST.LVQ?consumer.prefetchSize=1");
                destination = session->createTopic("ATADDRESS.1::Q1?consumer.prefetchSize=1");
                //destination = session->createTopic("TICKERPLAN.TICKER.#?consumer.prefetchSize=1");
            }
            else {
                destination = session->createQueue("ATADDRESS.1::Q1?consumer.prefetchSize=1");
            }
...
int main(int argc, char* argv) {
    ...
    activemq::library::ActiveMQCPP::initializeLibrary();
    {
        ...
        consumer = new HelloWorldConsumer(brokerURI, numMessages, useTopics, sessionTransacted);
