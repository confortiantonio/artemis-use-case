    ...
    virtual void run() {
        ...
        try {
            ...
            string text = (string)"Hello world! from thread " + threadIdStr;
            {
                ...
                message->setStringProperty("SUBJECT", "bv.quote.2.id");
                producer->send(message.get());
            }
            {
                std::auto_ptr<TextMessage> message(session->createTextMessage(text));                
                message->setLongProperty("MESSAGE_NUMBER", 1);
                message->setStringProperty("MESSAGE_KEY", "1");
                message->setStringProperty("SUBJECT", "cmf.quote.2.id");
                producer->send(message.get());
            }

            {
                std::auto_ptr<TextMessage> message(session->createTextMessage(text));
                message->setLongProperty("MESSAGE_NUMBER", 2);
                message->setStringProperty("MESSAGE_KEY", "2");
                message->setStringProperty("SUBJECT", "cmf.quote.3.id");
                producer->send(message.get());
            }
            ...
            for (int ix = 0; ix < numMessages; ++ix) {
                ...
                message->setStringProperty("SUBJECT", "bv.order.2.id");
