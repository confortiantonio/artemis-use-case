...
public:
    ...
    HelloWorldConsumer(const std::string& brokerURI, const std::string& address, const std::string& filter, int numMessages, bool useTopic = false, bool sessionTransacted = false, int waitMillis = 15 * 60 * 1000) :
        latch(1),
    ...
    virtual void run() {
        ...
        try {
            ...
            if (useTopic) {
                destination = session->createTopic(address); //"TICKERPLAN::LVQ?consumer.prefetchSize=1"
            }
            else {
    ...
    virtual void onMessage(const Message* message) {
        ...
        try {
            ...
            string messageKey = message->getStringProperty("MESSAGE_KEY");
            string subject = message->getStringProperty("SUBJECT");
            long messageNumber = message->getLongProperty("MESSAGE_NUMBER");

            printf("Message #%d Received: body<%s> key<%s> messageNumber<%d> subject<%s>\n", count, text.c_str(), messageKey.c_str(), messageNumber, subject.c_str());
...
int main(int argc, char* argv[]) {
    ...
    activemq::library::ActiveMQCPP::initializeLibrary();
    {
        ...
        consumer = new HelloWorldConsumer(brokerURI, address, filter, numMessages, useTopics, sessionTransacted);
