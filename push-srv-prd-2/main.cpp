...
public:
    ...
    virtual void run() {
        ...
        try {
            ...
            if (useTopic) {
                destination = session->createTopic("ATADDRESS.1");
            }
            else {
            ...
            std::auto_ptr<TextMessage> message2(session->createTextMessage(text));
            message2->setLongProperty("MESSAGE_NUMBER", 0);
            message2->setStringProperty("MESSAGE_KEY", "1");
