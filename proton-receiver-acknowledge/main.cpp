...
struct worker_message {
    /*
     * Provide ongoing (limited) access to Proton objects used outside of a callback.
     * Deliberately non-copyable to help prevent thread safety problems.
     *
     * Construct in on_message() callback.
     * Destruct in delivery work_queue or a callback of the delivery's connection.
     *
     * dlv_ptr_ points to a Proton reference-counted accessor to an internal Proton object.  The
     * application cannot use mutexes to guaranty thread safe use of or copying of dlv_ptr_ while
     * the engine is running.  Do not access the underlying delivery except when processing a
     * callback from the connection or as work on its work_queue.
     *
     * msg_ptr_ points to the inbound message data (move semantics).  The application can use it in
     * any thread with appropriate mutex use.
     */

     // Call from on_message() callback
    worker_message(proton::delivery& dlv, proton::message& msg)
        : dlv_ptr_(new proton::delivery(dlv)),
        msg_ptr_(new proton::message()) {
        swap(*msg_ptr_, msg); // 
    }

    // Only call from the connection's work_queue or one of its callbacks
    ~worker_message() {}

    proton::delivery* dlv() { return dlv_ptr_.get(); }
    proton::message* msg() { return msg_ptr_.get(); }
    std::unique_ptr<proton::delivery> dlv_ptr_;
    std::shared_ptr<proton::message> msg_ptr_;
};
...
public:
    ...
    void safe_delete(worker_message* wm) {
        std::unique_lock<std::mutex> l{ lock_ };
...
private:
    ...
    void on_message(proton::delivery& dlv, proton::message& msg) override {
        ...
        // Note: the proton::message is now empty, its contents are now in the worker_message.
        messages_.push(wm);
    ...
    try {
        ...
        std::thread receiver([&]() {
            for (int i = 0; i < desired; ++i) {
                ...
                proton::delivery* dlv = wm->dlv();
                proton::message* msg = wm->msg();
                std::string messageKey = proton::get<std::string>(msg->properties().get("MESSAGE_KEY"));
                long messageNumber = proton::get<long>(msg->properties().get("MESSAGE_NUMBER"));
                //Message #%d Received: body<%s> key<%s> messageNumber<%d>
                OUT(std::cout << "RECEIVE: Received message '" << msg->body() << " key " << messageKey << " messageNumber " << messageNumber << "'\n");
